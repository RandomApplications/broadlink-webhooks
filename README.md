...
To use these Webhooks Applets to turn a BroadLink device on or off or to activate a BroadLink scene, you'll call an IFTTT URL like this: `https://maker.ifttt.com/trigger/BroadLink-On+This_Device_Name/with/key/YOUR_IFTTT_WEBHOOKS_KEY`
...
* Open All IFTTT Edit URLs for Webhooks Applets Created by `broadlink-webhooks` *(Edit Applet URLs will open in your default web browser. You should be logged into IFTTT in your default web browser before running this task.)*
...
**Actually finally, you'll be prompted for your IFTTT username and password to be able to log into IFTTT.** If you have Two-Step Verification enabled in IFTTT, you will be prompted for a two-step verification code before being able to log in. *While `broadlink-webhooks` fully supports two-step verification, it does not support logging into IFTTT using the Apple, Google, or Facebook login options. Only regular IFTTT accounts are supported.*
